...
const iconControllerConfig = require('./icons');

module.exports = {
  getFields: (object) => {
    object.maxWidth = object.maxWidth || 0;
    object.maxHeight = object.maxHeight || 0;
    object.maxFontSize = object.maxFontSize || 0;
    ...
    return [
      ...
      {prop: 'maxFontSize', min: 1, step: 1},
      {
        ...
        descriptor: {
          value: () => {
            ...
            icon.key = krot.makeUniqueName('icon', object.icons);
            icon.textObject = object;
            object.icons[icon.key] = icon;
            new krot.Controller(icon, iconControllerConfig);
          },
        },
        export() {
          return Object.entries(object.icons).reduce((acc, [key, icon]) => {
            acc[key] = {
              x: icon.x,
              y: icon.y,
              scale: {
              ...
              texture: icon.texture.textureCacheIds[0] || '',
            };
      ...
      ...Object.entries(object.icons).map(([key, icon]) => ({
        prop: key,
        descriptor: {
          value: () => {
            new krot.Controller(icon, iconControllerConfig);
